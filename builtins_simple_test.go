...
func TestBuiltinHandler_PWD(t *testing.T) {
	state := NewShellState()
	handler := NewBuiltinHandler(state)

	result := handler.pwd([]string{})
	
	if result.ExitCode != 0 {
		t.Errorf("Exit code should be 0 for pwd, got %d", result.ExitCode)
	}
	
	if result.Error != nil {
		t.Errorf("Error should be nil for successful pwd, got %v", result.Error)
	}
	
	if result.Output != state.WorkingDirectory {
		t.Errorf("Output should match working directory. Expected %q, got %q", 
			state.WorkingDirectory, result.Output)
	}
}
...
func TestBuiltinHandler_Help_Command(t *testing.T) {
	...
	commands := []string{"cd", "pwd", "exit", "help"}
